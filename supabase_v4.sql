...
CREATE POLICY "rep owns representadas" ON representadas
...
CREATE POLICY "rep owns financeiro" ON financeiro
  FOR ALL USING (rep_id = (SELECT id FROM representantes WHERE email = auth.email()));
...
CREATE POLICY "rep owns impostos" ON impostos
  FOR ALL USING (rep_id = (SELECT id FROM representantes WHERE email = auth.email()));
...
CREATE POLICY "rep owns rotas" ON rotas
  FOR ALL USING (rep_id = (SELECT id FROM representantes WHERE email = auth.email()));
